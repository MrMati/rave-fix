...
void Songs::loadLiked(){
    ...
    for(Song* song : library){
        ...
        bool liked = settings.value(key, false).toBool();
        song->setLiked(liked);
    }
}
...
QString Songs::getSongTitleByUrl(const QUrl &songUrl) const {
    for (const auto &song : library) {
        if (song->getFileUrl() == songUrl) {
            return song->getName();
        }
    }
    return QString();
...
QString Songs::getSongAuthorByUrl(const QUrl &songUrl) const {
    for (const auto &song : library) {
        if (song->getFileUrl() == songUrl) {
